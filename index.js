...
function processLastItem(stringList, callback) {
  const last = callback(stringList[stringList.length-1]);
  return last
}
...
function processContains(item, list, callback) {
  const contain = callback(list.includes(item));
  return contain
}
...
runners.forEach(function(items){
  return getNames.push(`${items.last_name}, ${items.first_name}`);
});
  return getNames
}
...
function firstNamesAllCaps(runners) {
   const capNames = runners.map((name) => {
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const tSize = runners.filter((item) => item.shirt_size === tShirtSize);
    return tSize
  }
...
function tallyUpDonations(runners) {
  const sumAll = runners.reduce(function (accumulator, currentValue){
